...
TfLiteStatus FullyConnectedEval(TfLiteContext* context, TfLiteNode* node) {
  ...
  MicroPrintf("================================ FullyConnectedEval ================================");
  ...
  const auto& data =
      *(static_cast<const OpDataFullyConnected*>(node->user_data));
      // Check if per-channel quantization parameters are present. 
      // Both per_channel_output_multiplier and per_channel_output_shift must be 
      // non-nullptr for per-channel quantization to be used.
      ...
      switch (filter->type) {
        case kTfLiteInt4: {
          ...
          tflite::tensor_utils::UnpackDenseInt4IntoInt8(
              ...
              // If the filter is quantized with per-channel quantization, use the per-channel parameters.
          if (is_per_channel) {
...
TFLMRegistration Register_FULLY_CONNECTED() {
  ...
  MicroPrintf("======================= Register_FULLY_CONNECTED =======================");
...
TFLMInferenceRegistration RegisterInference_FULLY_CONNECTED() {
  ...
  MicroPrintf("RegisterInference_FULLY_CONNECTED");
